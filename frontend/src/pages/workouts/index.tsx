  ...
  return (
    <Layout title="Treinos 🏋️">
      ...
      <div className="flex items-center justify-between mb-6 gap-3 flex-wrap">
        ...
        <button onClick={() => showForm ? handleCloseForm() : setShowForm(true)} className="btn-primary">
      ...
      {showForm && (
        <div className="card mb-6 border-2 border-blue-200">
          <h3 className="font-black text-gray-900 mb-4">
            {editingWorkout ? "✏️ Editar Treino" : "💪 Novo Treino — Bora!"}
          </h3>
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-1">Exercício *</label>
              <input type="text" required value={form.exercise}
                onChange={(e) => setForm({ ...form, exercise: e.target.value })}
                className="input-field" placeholder="ex: Supino Reto" />
            </div>
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-1">Grupo Muscular *</label>
            ...
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-1">Séries *</label>
              <input type="number" required min={1} max={100} value={form.sets}
                onChange={(e) => setForm({ ...form, sets: Number(e.target.value) })}
                className="input-field" />
            </div>
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-1">Repetições *</label>
              <input type="number" required min={1} max={1000} value={form.reps}
                onChange={(e) => setForm({ ...form, reps: Number(e.target.value) })}
                className="input-field" />
            </div>
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-1">Carga (kg)</label>
            ...
            <div>
              ...
              <input type="text" value={form.notes}
                ...
                className="input-field" placeholder="ex: bateu um cansaço no final..." />
            </div>
        ...
        <>
          <div className="space-y-3">
            {(data?.items || []).map((w: any) => (
              <div key={w.id} className="card flex items-start justify-between gap-4 hover:border-blue-200 transition-colors">
                <div className="flex-1">
                  <div className="flex items-center gap-2 flex-wrap mb-1">
                    <h4 className="font-black text-gray-900">{w.exercise}</h4>
                  ...
                  <p className="text-gray-600">
                    {w.sets} séries × {w.reps} reps @ <strong className="text-blue-700">{w.weight_kg}kg</strong>
                    {w.weight_kg > 0 && (
                ...
                <div className="flex gap-1">
                  <button onClick={() => handleEdit(w)}
                    className="text-blue-500 hover:text-blue-700 px-2 py-1 hover:bg-blue-50 rounded-lg transition-colors text-lg">
                    ✏️
                  </button>
                  <button
                    onClick={() => { if (confirm("Deletar esse treino? 🐔")) deleteMutation.mutate(w.id); }}
                    className="text-red-400 hover:text-red-600 px-2 py-1 hover:bg-red-50 rounded-lg transition-colors text-lg">
                    🗑️
                  </button>
                </div>
              </div>
